async function reservarPremium() {
    try {
        ...
        .then(data1 => {
            ...
            if(data1.result == "OK") {
                ...
                const data2 = {"id": null, "nif":nif, "hotel": 'Melia-Madrid-Princesa', "destino": 'Madrid', "tipo": 'Premium', "huespedes": huespedes, "habitaciones":habitaciones, "fechaEntrada": checkInDefi, "fechaSalida": checkOutDefi};
...
async function reservarSuite() {
    try {
        var nif = await document.getElementById("nif-princesa2").value;
        var checkIn = await document.getElementById("date-in-princesa2").value;
        var checkOut = await document.getElementById("date-out-princesa2").value;
        var huespedes = await document.getElementById("guest-princesa2").value;
        var habitaciones = await document.getElementById("room-princesa2").value;
        var checkInDia = checkIn.substr(0,2);
        var checkOutDia = checkOut.substr(0,2);
        var checkInMes = checkIn.substr(3,(checkIn.indexOf(",")) - 3);
        var checkOutMes = checkOut.substr(3,(checkOut.indexOf(",")) - 3);
        ...
        .then(data1 => {
            ...
            if(data1.result == "OK") {
                ...
                var checkInDefi = new Date(checkInDef);
                var checkOutDefi = new Date(checkOutDef);
                const data2 = {"id": null, "nif":nif, "hotel": 'Melia-Madrid-Princesa', "destino": 'Madrid', "tipo": 'Suite', "huespedes": huespedes, "habitaciones":habitaciones, "fechaEntrada": checkInDefi, "fechaSalida": checkOutDefi};
                const address2 = "api/v1/reservas/insert";
                fetch(address2, {
                    method: 'POST',
                    headers: {
...
async function reservarGrandSuite() {
    try {
        var nif = await document.getElementById("nif-princesa3").value;
        var checkIn = await document.getElementById("date-in-princesa3").value;
        var checkOut = await document.getElementById("date-out-princesa3").value;
        var huespedes = await document.getElementById("guest-princesa3").value;
        var habitaciones = await document.getElementById("room-princesa3").value;
        var checkInDia = checkIn.substr(0,2);
        var checkOutDia = checkOut.substr(0,2);
        var checkInMes = checkIn.substr(3,(checkIn.indexOf(",")) - 3);
        var checkOutMes = checkOut.substr(3,(checkOut.indexOf(",")) - 3);
        ...
        .then(data1 => {
            ...
            if(data1.result == "OK") {
                ...
                var checkInDefi = new Date(checkInDef);
                var checkOutDefi = new Date(checkOutDef);
                const data2 = {"id": null, "nif":nif, "hotel": 'Melia-Madrid-Princesa', "destino": 'Madrid', "tipo": 'Grand-Suite-Presidencial', "huespedes": huespedes, "habitaciones":habitaciones, "fechaEntrada": checkInDefi, "fechaSalida": checkOutDefi};
                const address2 = "api/v1/reservas/insert";
                fetch(address2, {
                    method: 'POST',
                    headers: {
...
$('#reservaFormPremium').submit(function (e) {
    e.preventDefault();
});

$('#reservaFormSuite').submit(function (e) {
    e.preventDefault();
});
